...
from electrum_med import bmp, pyqrnative, BasePlugin
...
class QR_Window(QWidget):
    ...
    def set_content(self, addr, label, amount, currency):
        ...
        if amount:
            ...
            amount_text += "<span style='font-size: 21pt'>%s</span> <span style='font-size: 16pt'>MED</span> " % str(self.amount) 
        else:
            self.amount = None
        ...
        msg = 'mediterraneancoin:'+self.address
...
class Plugin(BasePlugin):
    ...
    def item_changed(self, item, column):
        ...
        #print text
        m = re.match('^(\d*(|\.\d*))\s*(|MED|EUR|USD|GBP|CNY|JPY|RUB|BRL)$', text)

...
def hash_160_to_bc_address(h160, addrtype=51):
...
def SecretToASecret(secret, compressed=False, addrtype=51):
    vchIn = chr((addrtype+128)&255) + secret
    if compressed: vchIn += '\01'
    return EncodeBase58Check(vchIn)

def ASecretToSecret(key, addrtype=51):
...
def msg_magic(message):
    ...
    return "\x18Mediterraneancoin Signed Message:\n" + encoded_varint + message

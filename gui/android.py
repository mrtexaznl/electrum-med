...
from electrum_med import SimpleConfig, Wallet, WalletStorage, format_satoshis, mnemonic_encode, mnemonic_decode
...
def pay_to(recipient, amount, fee, label):
    ...
    droid.dialogCreateSpinnerProgress("Electrum-MED", "signing transaction...")
    droid.dialogShow()
...
def make_new_contact():
    ...
    if r:
        ...
        if data:
            if re.match('^mediterraneancoin:', data):
        ...
        if event["name"]=="click":
            ...
            if out == 'contacts':
                ...
                if receive_addr:
                    ...
                    if amount:
                        receive_addr = 'mediterraneancoin:%s?amount=%s'%(receive_addr, amount)
        ...
        if event["name"] == "click":
            ...
            if id=="buttonPay":
                ...
                if not is_valid(recipient):
                    modal_dialog('Error','Invalid Mediterraneancoin address')
                    continue
...
class ElectrumGui:
    ...
    def restore_wallet(self):
        ...
        droid.dialogCreateSpinnerProgress("Electrum-MED", msg)
        droid.dialogShow()

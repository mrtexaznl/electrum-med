...
from electrum_med.util import get_resource_path as rsrc
from electrum_med.bitcoin import is_valid
...
import re
import time
from electrum_med.wallet import Wallet, WalletStorage
import webbrowser
import history_widget
import receiving_widget
from electrum_med import util
import datetime

from electrum_med.version import ELECTRUM_VERSION as electrum_version
from electrum_med.util import format_satoshis, age
...
class MiniWindow(QDialog):
    ...
    def __init__(self, actuator, expand_callback, config):
        ...
        self.address_input.setPlaceholderText(_("Enter a Mediterraneancoin address or contact"))
        ...
        self.setWindowTitle("Electrum-MED")
    ...
    def the_website(self):
        webbrowser.open("http://social.mediterraneancoin.org")
...
class ReceivePopup(QDialog):
    ...
    def setup(self, address):
        label = QLabel(_("Copied your Mediterraneancoin address to the clipboard!"))
        ...
        self.setWindowTitle("Electrum - " + _("Receive Mediterraneancoin payment"))
    ...
    def send(self, address, amount, parent_window):
        ...
        if dest_address is None or not is_valid(dest_address):
            QMessageBox.warning(parent_window, _('Error'), 
                _('Invalid Mediterraneancoin Address') + ':\n' + address, _('OK'))
            return False

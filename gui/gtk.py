...
from electrum_med.util import print_error
from electrum_med.bitcoin import is_valid
from electrum_med import mnemonic, pyqrnative, WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Electrum-MED"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from electrum_med.util import format_satoshis, parse_url
from electrum_med.network import DEFAULT_SERVERS
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Electrum-MED ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid mediterraneancoin address:\n"+to_address)
            return
